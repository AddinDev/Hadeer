...
struct TaskMapper {
  ...
  static func responseToDomain(_ responses: TaskResponses) -> [TaskModel] {
    return responses.map { response in
      return TaskModel(id: response.id, title: response.nama, grade: response.kelas, teacherName: response.namaGuru, teacherId: response.guruID, time: response.waktu.formatToDate(), material: response.materi, students: studentsResponseToModel(response.siswa))
    }
  }
